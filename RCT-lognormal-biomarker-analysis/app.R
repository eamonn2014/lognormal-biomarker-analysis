            ...
            tabsetPanel(type = "tabs",
                        ...
                        tabPanel( div(h4(tags$span(style="color:black", "Treatment effect"))),
                          h3("Estimate the treatment effect using ANCOVA model"),
                                 ...
                                 p('The log scale is not very intuitive so we back transform and present below. This is the result of interest, 
                                 on the more relevant original scale, the proportional change adjusted for baseline with 95% confidence (this is the ratio of adjusted
                                 geometric means (B/A)).
                        ...
                        h3("Estimate the adjusted means from ANCOVA model"),
                                 ...
                                 p('Geometric Mean of outcome measure treatment A. Note, the log of a mean is not the mean of a log. So we are not estimating a mean on the transformed scale.
  ...
  random.sample <- reactive({
    ...
    output$statement <-  renderPrint({
      ...
      cat(paste0("\nBut the two groups differed in their pre treatment version of the predictor, the patients treated in arm B were ", 
                 p3(abs(B.DIFF)),  " units ",word2, " \nthan the patients under treatment A, that is y mean = ", p3(B.BASE)," under treatment B and y mean = ", p3(A.BASE), " for treatment A."))
